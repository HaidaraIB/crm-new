...
export const SuppliersFilterDrawer = () => {
    const { isSupplierFilterDrawerOpen, setIsSupplierFilterDrawerOpen, t, supplierFilters, setSupplierFilters, suppliers } = useAppContext();
    ...
    return (
        <>
            <aside className={`fixed inset-y-0 end-0 z-50 flex h-full w-full max-w-xs flex-col bg-card dark:bg-dark-card border-s dark:border-gray-800 transform transition-transform duration-300 ease-in-out 
                                ${isSupplierFilterDrawerOpen ? 'translate-x-0' : 'translate-x-full rtl:-translate-x-full'}`}>
                <div className="flex items-center justify-between p-4 border-b dark:border-gray-800 h-16">
                    <h2 className="text-lg font-semibold">{t('filterSuppliers')}</h2>
                    <Button variant="ghost" className="p-1" onClick={() => setIsSupplierFilterDrawerOpen(false)}>
            ...
            {isSupplierFilterDrawerOpen && (
                <div
                    ...
                    aria-hidden="true"
                    onClick={() => setIsSupplierFilterDrawerOpen(false)}
                ></div>
            )}
        </>
    );
