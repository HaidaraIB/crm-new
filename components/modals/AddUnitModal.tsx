...
export const AddUnitModal = () => {
    const { isAddUnitModalOpen, setIsAddUnitModalOpen, t, setIsSuccessModalOpen, setSuccessMessage, currentUser } = useAppContext();
    ...
    const isLoading = addUnitMutation.isPending;
    ...
    useEffect(() => {
        if (isAddUnitModalOpen && projects.length > 0) {
            setFormState(prev => {
                // إذا كان project فارغ، اختر الأول (استخدم ID بدلاً من name)
                if (!prev.project) {
                    return { ...prev, project: projects[0].id.toString() };
                }
                return prev;
            });
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Prepare unit data with project ID (not name) and company ID
            const unitData = {
                name: formState.name,
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Input
                            id="code"
                            placeholder={t('enterCode') || 'Leave blank to auto-generate'}
                            value={formState.code}
                            onChange={handleChange}
                            className={errors.code ? 'border-red-500 dark:border-red-500' : ''}
                        />
                        {errors.code && (
