...
export const EditChannelModal = () => {
    const { isEditChannelModalOpen, setIsEditChannelModalOpen, t, editingChannel, setEditingChannel, channelTypes, setIsSuccessModalOpen, setSuccessMessage, language, currentUser } = useAppContext();
    ...
    // Also include the current channel's type if it's not already in the list
    const availableChannelTypes = React.useMemo(() => {
        const combined = new Set([...apiChannelTypes, ...DEFAULT_CHANNEL_TYPES]);
        // Include the current editing channel's type if it exists and isn't in the list
        if (editingChannel?.type && !combined.has(editingChannel.type)) {
            combined.add(editingChannel.type);
        }
        return Array.from(combined);
    }, [apiChannelTypes, editingChannel]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            handleClose();
            setSuccessMessage(t('channelUpdatedSuccessfully') || 'Channel updated successfully!');
    ...
    return (
        <Modal isOpen={isEditChannelModalOpen} onClose={handleClose} title={t('editChannel') || 'Edit Channel'}>
