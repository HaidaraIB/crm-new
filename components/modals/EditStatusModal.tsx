...
export const EditStatusModal = () => {
    const { isEditStatusModalOpen, setIsEditStatusModalOpen, t, editingStatus, setEditingStatus, language, setIsSuccessModalOpen, setSuccessMessage, currentUser } = useAppContext();
    ...
    useEffect(() => {
        if (editingStatus) {
            // Convert category from API format (e.g., "Active", "Follow Up") to lowercase with underscores
            let categoryValue = editingStatus.category?.toLowerCase() || 'active';
            // Handle "Follow Up" -> "follow_up"
            if (categoryValue === 'follow up' || categoryValue === 'followup') {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            handleClose();
            setSuccessMessage(t('statusUpdatedSuccessfully') || 'Status updated successfully!');
    ...
    return (
        <Modal isOpen={isEditStatusModalOpen} onClose={handleClose} title={t('editStatus') || 'Edit Status'}>
