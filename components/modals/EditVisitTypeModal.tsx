...
export const EditVisitTypeModal = () => {
    const {
        isEditVisitTypeModalOpen,
    ...
    const handleClose = () => {
        setIsEditVisitTypeModalOpen(false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            setErrors({ _general: error?.message || t('failedToUpdateVisitType') || 'Failed to update visit type.' });
        }
    };

    if (!editingVisitType) return null;

    return (
        <Modal isOpen={isEditVisitTypeModalOpen} onClose={handleClose} title={t('editVisitType') || 'Edit visit type'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className={`flex ${language === 'ar' ? 'flex-row-reverse' : ''} justify-end gap-2`}>
                    ...
                    <Button type="submit" disabled={loading}>{loading ? (t('loading') || 'Loading…') : (t('saveChanges') || 'Save')}</Button>
                </div>
            </form>
        </Modal>
    );
