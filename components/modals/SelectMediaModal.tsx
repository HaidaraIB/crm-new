        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 text-sm -mb-px transition-colors ${activeTab === tab.id ? PAGE_TAB_ACTIVE : PAGE_TAB_INACTIVE}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === 'upload' && (
          <div className="rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 p-8 text-center">
            <input
              ...
              id="media-upload"
            />
            <label htmlFor="media-upload" className="cursor-pointer flex flex-col items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-primary-700 dark:hover:text-primary-200">
