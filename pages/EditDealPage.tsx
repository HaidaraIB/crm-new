...
export const EditDealPage = () => {
    ...
    const projects = projectsResponse?.results ?? EMPTY_LIST;
    ...
    const users = Array.isArray(usersResponse)
        ? usersResponse
        : (usersResponse?.results ?? EMPTY_LIST);

    const updateDealMutation = usePatchDeal();
    ...
    const buildDealPayload = (
        ...
        calc: { discountAmount: number; totalValue: number; salesCommissionAmount: number }
    ): Record<string, unknown> => {
        const payload: Record<string, unknown> = {
            client: state.leadId,
            employee: Number(state.employee) || currentUser?.id || null,
            started_by: Number(state.startedBy) || currentUser?.id || null,
            closed_by: Number(state.closedBy) || currentUser?.id || null,
            payment_method: state.paymentMethod.toLowerCase() || null,
            status: state.status.toLowerCase(),
            stage: state.stage,
            value: calc.totalValue,
            start_date: state.startDate || null,
            closed_date: state.closedDate || null,
            reminder_date: state.reminderDate || null,
            discount_percentage: Number(state.discountPercentage) || 0,
            discount_amount: calc.discountAmount,
            sales_commission_percentage: Number(state.salesCommissionPercentage) || 0,
            sales_commission_amount: calc.salesCommissionAmount,
        ...
        if (isRealEstate) {
            if (state.project) {
                const projectObj = projects.find((p: any) => p.name === state.project || p.id.toString() === state.project);
                payload.project = projectObj ? projectObj.id : Number(state.project);
            }
            if (state.unit) {
                const unitObj = allUnits.find((u: any) => u.code === state.unit || u.id.toString() === state.unit);
                payload.unit = unitObj ? unitObj.id : Number(state.unit);
            }
        }
    ...
    const computeDealCalculatedValues = (state: typeof formState) => {
        const value = parseFloat(state.value) || 0;
        const discountPercent = parseFloat(state.discountPercentage) || 0;
        const discountAmount = value * (discountPercent / 100);
        const totalValue = value - discountAmount;
        const commissionPercent = parseFloat(state.salesCommissionPercentage) || 0;
        const salesCommissionAmount = totalValue * (commissionPercent / 100);
        return { discountAmount, totalValue, salesCommissionAmount };
    };
    ...
    useEffect(() => {
        ...
        const paymentMethodDisplay = paymentMethodRaw.charAt(0).toUpperCase() + paymentMethodRaw.slice(1).toLowerCase();

        const statusRaw = editingDeal.status || 'reservation';
        const statusDisplay = statusRaw.charAt(0).toUpperCase() + statusRaw.slice(1).toLowerCase();
        ...
        initialPayloadRef.current = buildDealPayload(initState, computeDealCalculatedValues(initState));
        setErrors({});
        setInitialized(true);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [editingDeal, projects, allUnits, currentUser?.id, isRealEstate]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const next = buildDealPayload(formState, calculatedValues);
    ...
    if (!editingDeal) {
        return (
            <PageWrapper title={t('editDeal') || 'Edit Deal'}>
                <Card>
                    <div className="text-center py-8">
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                            {t('noDealSelected') || 'No deal selected for editing.'}
                        </p>
                        <Button onClick={goBackToDeals}>
                            {t('backToDeals') || 'Back to Deals'}
                        </Button>
                    </div>
                </Card>
            </PageWrapper>
        ...
        <PageWrapper
            title={
                <div className="flex items-center gap-3">
                    ...
                    <span>{t('editDeal') || 'Edit Deal'}</span>
                </div>
            }
        >
            <form onSubmit={handleSubmit} onKeyDown={(e) => {
                ...
                <Card>
                    ...
                    <div className="mt-6 flex justify-end gap-2">
                        <Button type="button" variant="secondary" onClick={goBackToDeals} disabled={loading}>
                            {t('cancel')}
                        </Button>
